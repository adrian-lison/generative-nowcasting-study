...
```{r}
here::i_am("code/nowcast_synthetic.Rmd")
...
models <- list(
  ...
  miss_stepwise_independent = list(
    model = "impute_independent_then_adjust_renewal",
    data = "incomplete"
  ),
  R_generative_ets = list(
    model = "impute_adjust_renewal", data = "complete"
  ),
  R_stepwise_ets = list(
    model = "impute_adjust", data = "complete"
  )
)
...
```{r}
for (wave_i in 1:2) {
  scenario_name <- c("wave1", "wave2")[wave_i]
