...
plot1 <- bind_rows(list(
  ...
  arrange(timepoint) %>% 
  mutate(timepoint = forcats::fct_inorder(format(timepoint - 1 + zoo::as.Date(0), "%B %d"), ordered = T)) %>% 
  ggplot(aes(x = delay, y = p)) +
...
plot2 <- data.frame(mean_dist = dist_get_mean(symp_to_rep_dist[[3]])) %>% mutate(date = zoo::as.Date(0) + (0:(n()-1))) %>% 
